...
def _build_strategy_blueprints(topic_names: dict[str, str]) -> tuple[StrategyBlueprint, ...]:
    ...
    strategies = (
        ...
        StrategyBlueprint(
            ...
            family="极速与动能模板",
            name="收线试驾 / 逆 1 失败、顺 1 成功",
            status=STRATEGY_STATUS_DESIGNED,
            summary="把收线试驾和逆1失败顺1合并成 Brooks 的动能延续模板：要么直接顺势收线试驾，要么等第一次失败逆势尝试后再顺势恢复。",
            steps=(
                ...
                _step("setup_context", STEP_STATUS_DESIGNED, "突破有明显跟进时可直接收线试驾；若第一次逆势尝试失败，则顺势恢复更容易成功。", "course_15A", "course_15B", "course_09A", "course_09B", "supp_breakout"),
                _step("signal_bar", STEP_STATUS_DESIGNED, "趋势 bar 收在线外侧且实体强，或顺 1 恢复 bar 明显强于逆 1。", "course_08A"),
                _step("entry_trigger", STEP_STATUS_DESIGNED, "可分收线追入和高/低 1 stop entry 两类。"),
                _step("trigger_invalidation", STEP_STATUS_DESIGNED, "若出现首根明确回调 bar、突破点回测失败或逆势尝试转强，则退出动能模板。"),
                ...
                _step("reentry_addon", STEP_STATUS_DESIGNED, "逆 1 失败、顺 1 成功本身就是这套模板的再入场变体。"),
            ),
            tuning_notes="太妃的“收线试驾”和“逆1顺1”本质都是 Brooks 动能延续模板的不同入口，不再单独拆蓝图。",
            source_refs=(
                str((REPO_DIR / "策略资料" / "太妃价格行为" / "L13A - 急速交易 - ✨收线.md").resolve()),
                str((REPO_DIR / "策略资料" / "太妃价格行为" / "L13B - 急速交易 - ✨逆1顺1.md").resolve()),
                str(FOUNDATION_INDEX_PATH),
            ),
        ),
        StrategyBlueprint(
        ...
        StrategyBlueprint(
            ...
            name="双重顶底 / 楔形 / MTR",
            status=STRATEGY_STATUS_DESIGNED,
            summary="把双重顶底、楔形和 MTR 合并成同一套 Brooks 反转模板，避免把同一套反转语义拆成多个重复蓝图。",
            steps=(
                ...
                _step("setup_context", STEP_STATUS_DESIGNED, "逆向压力积累、双重测试失败、三推衰竭、趋势线突破和测试否决应视作同一套反转语义。", "course_21A", "course_22A", "course_22B", "course_22C", "course_22D", "course_24A", "course_24B", "course_24C", "course_24D", "course_24E", "supp_double_top_bottom", "supp_wedge_exhaustion"),
            ...
            tuning_notes=(
                "1. 双重顶底、楔形和 MTR 不再拆成多个平行模板，而是共用同一套反转底层模块。\n"
                "2. 这类策略要单独处理逆向压力积累、趋势线突破、极值不再递进和测试否决。"
            ),
            source_refs=(
                str(FOUNDATION_INDEX_PATH),
                str(PRACTICAL_INDEX_PATH),
                str(BEST10_INDEX_PATH),
                str((REPO_DIR / "策略资料" / "太妃价格行为" / "L14A - 旗形回调.md").resolve()),
                str((REPO_DIR / "策略资料" / "太妃价格行为" / "L14B - ✨双重顶底-✨楔形.md").resolve()),
                str((REPO_DIR / "策略资料" / "太妃价格行为" / "L16A - 反转交易.md").resolve()),
            ),
        ),
        StrategyBlueprint(
